...
RSpec.describe do
  context 'api' do
    it do
      ...
      api = SweetMoon::API.new(shared_object: config['jit:2.0.5']['shared_object'])
  ...
  context 'state' do
    it do
      ...
      expect(state.meta.runtime).to eq('LuaJIT 2.0.5 (Lua 5.1)')
      ...
      expect(state.add_package_path(config['jit:2.0.5']['fennel'])).to be_nil
  ...
  context 'state configs' do
    it do
      config = YAML.load_file('config/tests.yml')

      state = SweetMoon::State.new(
        shared_object: config['jit:2.0.5']['shared_object'],
        api_reference: '5.1.4',
        interpreter: '5.1'
      )
