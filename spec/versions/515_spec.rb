...
RSpec.describe do
  context 'api' do
    it do
      ...
      api = SweetMoon::API.new(shared_object: config['5.1.5']['shared_object'])
  ...
  context 'state' do
    it do
      ...
      expect(state.meta.runtime).to eq('Lua 5.1')
      ...
      expect(state.add_package_path(config['5.1.5']['fennel'])).to eq(nil)
