  ...
  context '5.4.4' do
    ...
    context 'load, eval and read' do
      it do
        ...
        expect(@state.get('depth')).to be(true)
        expect(@state.get('name')).to eq('config lua')
        expect(@state.get('empty')).to be_nil
        ...
        expect(@state.eval('lorem = "spec amet";')).to be_nil

        expect(@state.get('lorem')).to eq('spec amet')

        expect(@state.eval('lorem = "spec2";')).to be_nil

        expect(@state.get('lorem')).to eq('spec2')

        expect(@state.destroy).to be_nil
    ...
    context 'write' do
      it do
        expect(@state.get('a')).to be_nil

        expect(@state.set('a', 200)).to be_nil
        expect(@state.get('a')).to eq(200)

        expect(@state.set('b', 1.5)).to be_nil
        expect(@state.get('b')).to eq(1.5)
        expect(@state.eval('return b')).to eq(1.5)

        expect(@state.set('c', true)).to be_nil
        expect(@state.get('c')).to be(true)

        expect(@state.set('d', false)).to be_nil
        expect(@state.get('d')).to be(false)

        expect(@state.set('e', nil)).to be_nil
        expect(@state.get('e')).to be_nil

        expect(@state.set('f', 'lorem ipsum')).to be_nil
        expect(@state.get('f')).to eq('lorem ipsum')

        expect(@state.set('h', :amet)).to be_nil
    ...
    context 'functions and lambdas' do
      it do
        expect(@state.set('rubyFn', ->(a, b) { a + b })).to be_nil

        expect(@state.eval('return rubyFn(1.2, 2);')).to eq(3.2)

        expect(@state.eval('function luaFn(a, b) return a + b; end')).to be_nil
    ...
    context 'tables, arrays and hashes' do
      it do
        ...
        expect(@state.set('b', ['a', 1, [2, 3, 4], { a: ['c', :d] }, '5'])).to be_nil
        ...
        expect(@state.set('rubyFn', ->(list) { list.join(',') })).to be_nil
        expect(@state.eval('return 1.5 + 1.5;')).to eq(3.0)
        expect(@state.eval(
        ...
        expect(@state.set('subFn', [{ a: ->(c, d) { c + d } }])).to be_nil
    ...
    context 'other types' do
      it do
        expect(@state.eval('a = coroutine.create(function() end)')).to be_nil
        expect(@state.get(:a)).to match(/^thread: 0x\d+$/)

        expect(@state.set(:b, Thread.new { 1 + 1 })).to be_nil
        expect(@state.get('b')).to match(/^#<Thread:0x.+>$/)

        expect(@state.eval('b = { c = coroutine.create(function() end) }')).to be_nil

        expect(@state.get(:b).keys).to eq(['c'])
        expect(@state.get(:b)['c']).to match(/^thread: 0x\d+$/)

        expect(@state.set(:c, { d: Thread.new { 1 + 1 } })).to be_nil
